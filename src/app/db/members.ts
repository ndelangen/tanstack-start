...
export async function list() {
  ...
  return {
    approved: async (groupId: string) =>
      base().filter('group_id', 'eq', groupId).filter('status', 'eq', 'approved'),
    pending: async (groupId: string) =>
      base().filter('group_id', 'eq', groupId).filter('status', 'eq', 'pending'),
    rejected: async (groupId: string) =>
      base().filter('group_id', 'eq', groupId).filter('status', 'eq', 'rejected'),
  };
}
